...
////////////////////
            ...
            switch (action.toLowerCase().trim()) {
                ...
                case "search":
                    if (phoneBooks.isEmpty()) {
                        System.out.println("Нечего искать. Массив пустой");
                    } else {
                ...
                case "print":
                    ...
                    if (phoneBooks.size() == 0) {
                        System.out.println("Нечего выводить, Массив пустой");
                    } else {
                ...
                case "edit":
                    ...
                    if (phoneBooks.size() == 0) {
                        System.out.println("Нечего редактировать, Массив пустой");
                    } else {
                ...
                case "load":
                    ...
                    System.out.println(phoneBooks.get(0).getClass().getName());
    ...
    private static void editNumberCollection(List<PhoneBooks> phoneBooks) {
        System.out.println("Для редактирования, укажите номер строки:");
        int sizeBooks = 0;
        int input;
        ListIterator<PhoneBooks> phoneBooksListIterator = phoneBooks.listIterator();
        while (phoneBooksListIterator.hasNext()) {
            System.out.println(sizeBooks + " " + phoneBooksListIterator.next());
            sizeBooks++;
        }
        try {
            input = Integer.valueOf(in.nextLine());
            if (input < phoneBooks.size()) {
                editElem(input, phoneBooks);
            }
    ...
    private static void editElem(int editNumCollection, List<PhoneBooks> phoneBooks) {
        String tempLine;
        String tempChange;
        int[] tempPhonesNumMass;
        System.out.println("Выберите, какое поле редактируем:");
        System.out.println("1 - ФИО:");
        System.out.println("2 - дата рождения(формат: yyyy-MM-dd");
        System.out.println("3 - номера телефонов:");
        System.out.println("4 - адресс");
        tempLine = in.nextLine();
        System.out.println("Вы выбрали поле: " + tempLine + ". Введите значение для замены:");
        try {
            tempChange = in.nextLine();
            switch (Integer.valueOf(tempLine)) {
                case 1: //ФИО
                    phoneBooks.get(editNumCollection).setF_I_O(tempChange);
                    successfullReplaced(editNumCollection, phoneBooks);

                    break;
                case 2: // Дата рождения
                    phoneBooks.get(editNumCollection).setB_Day(LocalDate.parse(tempChange.replaceAll("[^\\d-]", ""), DateTimeFormatter.ofPattern("yyyy-MM-dd")));
                    successfullReplaced(editNumCollection, phoneBooks);
                    break;
                case 3: // Номера телефонов)
                    tempPhonesNumMass = Arrays.stream(tempChange.replaceAll("[^\\d,]", "").split(","))
                            .mapToInt(Integer::parseInt).toArray();
                    phoneBooks.get(editNumCollection).setPhones(tempPhonesNumMass);
                    successfullReplaced(editNumCollection, phoneBooks);
                    break;
                case 4: //адресс
                    phoneBooks.get(editNumCollection).setAddress(tempChange);
                    successfullReplaced(editNumCollection, phoneBooks);
                    break;
    ...
    private static void sortCollection(int temp, List<PhoneBooks> phoneBooks) {
        ...
        switch (temp) {
            //По ФИО
            case 1:
                Collections.sort(phoneBooks, new PhoneBooks.PhoneBooksFIOCompare());
                System.out.println(phoneBooks);
            ...
            case 2:
                //   По дате рождения
                Collections.sort(phoneBooks, new PhoneBooks.PhoneBooks_B_DAY_Compare());
                System.out.println(phoneBooks);
                break;

            case 3:
                //По ФИО и дате рождения
                Collections.sort(phoneBooks, new PhoneBooks.PhoneBooksFIOCompare().thenComparing(new PhoneBooks.PhoneBooks_B_DAY_Compare()));
                System.out.println(phoneBooks);
                break;
            case 4:
                // По номерам телефонов"
                Collections.sort(phoneBooks, new PhoneBooks.PhoneBooksPhoneCompare());
                System.out.println(phoneBooks);
                break;
            case 5:
                // По адресам"
                Collections.sort(phoneBooks, new PhoneBooks.PhoneBooksAdressCompare());
                System.out.println(phoneBooks);
                break;
            default:
                System.out.println("Вы ввели неверное значение, повторите попытку");
    ...
    private static void dellElem(List<PhoneBooks> phoneBooks) {
        try {
            int temp = 0;
            Iterator iter = phoneBooks.listIterator();
            System.out.println("Выберите номер элемента, который необходимо удалить:");
            for (int i = 0; i < phoneBooks.size(); i++) {
                System.out.println("Элемент массива:" + i + iter.next().toString());
            }
            temp = Integer.valueOf(in.nextLine());
            if (temp > phoneBooks.size() - 1) {
                System.out.println("Вы указали неверное значение для удаления. попробуйте сначала...");
                startMessage();
            } else {
                phoneBooks.remove(temp);
                System.out.println("Элемент " + temp + " удалили");
            }
...
//-----------------------------------------------------------------------//
            ...
            phones = in.nextLine().replaceAll("[^\\d,]", "");
            phonesMass = Arrays.stream(phones.split(","))
...
//-----------------------------------------------------------------------//
 ...
 /*  Каждый элемент должен иметь:
    ...
    private static void startMessage() {
        ...
        System.out.println("             print - вывод всех записей с сортировкой по указанному полю (можно ограничиться двумя на выбор) ");
